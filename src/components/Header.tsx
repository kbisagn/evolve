...
import { Menu, Bell, UserPlus, BookOpen, Home, MapPin, Users, IndianRupee, BarChart3, X } from 'lucide-react';
...
export default function Header({ pageTitle }: HeaderProps) {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, text: 'New member "John Doe" signed up.', time: '5m ago', read: false },
    { id: 2, text: 'Subscription for "Jane Smith" is expiring tomorrow.', time: '1h ago', read: false },
    { id: 3, text: 'Expense "New Chairs" for ₹5000 added.', time: '3h ago', read: false },
    { id: 4, text: 'Server maintenance scheduled for tonight.', time: '1d ago', read: true },
  ]);
            ...
            {notificationsOpen && (
              <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-50 animate-fade-in-down">
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.map(n => (
                    <div key={n.id} className={`p-4 border-b border-gray-50 text-sm hover:bg-gray-50 ${!n.read ? 'bg-blue-50/50' : ''}`}>
                      <p className="text-gray-800">{n.text}</p>
                      <div className="flex justify-between items-center mt-2">
                        <span className="text-xs text-gray-400">{n.time}</span>
                        {!n.read && <button onClick={() => handleReadNotification(n.id)} className="text-xs text-blue-600 font-semibold hover:underline">Mark as read</button>}
                      </div>
                    </div>
                  ))}
                  {notifications.length === 0 && <p className="p-4 text-center text-gray-500">No new notifications.</p>}
                </div>
                <div className="p-2 bg-gray-50 text-center"><Link href="#" className="text-sm font-medium text-blue-600 hover:underline">View all notifications</Link></div>
              </div>
            )}
          </div>
        </div>
